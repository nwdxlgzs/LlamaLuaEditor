 ...
 */
package com.LlamaLuaEditor.common;

import static com.LlamaLuaEditor.aluasyntax.lparser.VLOCAL;
import static com.LlamaLuaEditor.aluasyntax.lparser.VUPVAL;
import static com.LlamaLuaEditor.aluasyntax.lparser.VVOID;

import android.graphics.Rect;
import android.util.Log;

import com.LlamaLuaEditor.LuaLexer;
import com.LlamaLuaEditor.LuaTokenTypes;


import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;
//import static com.LlamaLuaEditor.common.LuaTokenTypes;
    ...
    private class LexThread extends Thread {
        ...
        private void tokenize() {
            ...
            if (rowCount * 5 < maxRow) {
                if (!AluaParser.lexer(new DocumentProvider(hDoc), _abort) && _tokens != null && _tokens.size() > 4) {
...
//            HashMap<String, ArrayList<Pair>> locals = AluaParser.getLocalMap();
            HashMap<String, ArrayList<Pair>> values = AluaParser.getValueMap();

            LuaLexer lexer = new LuaLexer(hDoc);
            Language language = Lexer.getLanguage();
            language.clearUserWord();
            AluaParser.clearUserWord();
            try {
                int idx = 0;
                ...
                while (!_abort.isSet()) {
                    ...
                    if (isModule && lastType == LuaTokenTypes.STRING && type != LuaTokenTypes.STRING) {
                        ...
                        if (bul.length() > 2) {
                            ...
                            AluaParser.addUserWord(na + " :import");
                        }
                    ...
                    lastLen = len;
                    //mod by nwdxlgzs(没办法啊，LuaLexer没更新
                    switch (type) {
                        case DO:
                        ...
                        case FINALLY:
                            //关键字
                            if (lastType == LuaTokenTypes.DOT || lastType == LuaTokenTypes.LET) {
                                tokens.add(new Pair(len, NORMAL));
                                break;
                            }
                            tokens.add(new Pair(len, KEYWORD));
                        ...
                        case __ASM__:
                            //函数名
                            tokens.add(new Pair(len, KEYWORD));
                            language.addUserWord("nwdxlgzs为前瞻版提供的内联汇编方案有权无偿索要内联汇编实现以改进功能或进行审查");
                            String[] AsmMethods = {"iABC", "iABx", "iAsBx", "iAx", "bins", "pass", "setval"};
                            for (String s : AsmMethods) {
                                language.addUserWord(s);
//                                AluaParser.addUserWord(s + " :asm");
                            }
                            break;
                        case LCURLY:
                        ...
                        case NAME:
                            ...
                            if (lastType == LuaTokenTypes.FUNCTION) {
                                ...
                                AluaParser.addUserWord(name + " :function");
                            } else if (language.isUserWord(name)) {
                                tokens.add(new Pair(len, LITERAL));
                            } else if (lastType == LuaTokenTypes.GOTO) {
                                tokens.add(new Pair(len, LITERAL));
                            } else if (lastType == LuaTokenTypes.MUL && lastType3 == LuaTokenTypes.LOCAL) {
                            ...
                            if (lastType != LuaTokenTypes.DOT) {
                                ...
                                if (!loc && values.containsKey(name)) {
                                    ...
                                    for (Pair l : ls) {
//                                        System.out.println("l.getFirst()[" + l.getFirst() + "] == idx[" + idx + "]");
                                        if (l.getFirst() == idx) {
                                            Pair p = tokens.get(tokens.size() - 1);
                                            int tp = l.getSecond();
                                            if (tp == VVOID) {
                            ...
                            if (lastType == LuaTokenTypes.ASSIGN && name.equals("require")) {
                                ...
                                AluaParser.addUserWord(lastName + " :require");
                                if (lastNameIdx >= 0) {
